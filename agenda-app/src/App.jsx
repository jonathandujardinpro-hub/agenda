...
export default function App(){
  ...
  // Auto-save to Supabase with debounce (1.5s after last change)
  const sync = useCallback((evts, tasks, notes) => {
    if(!loaded) return;
    clearTimeout(saveTimer.current);
    setSyncing(true);
    saveTimer.current = setTimeout(async ()=>{
      await saveData({ evts, tasks, notes });
      setSyncing(false);
    }, 1500);
  }, [loaded]);

  useEffect(()=>{ if(loaded) sync(evts,tasks,notes); },[evts,tasks,notes,loaded]);
  ...
  return (
    <div style={{minHeight:"100vh",background:BG,fontFamily:"'SF Pro Display',-apple-system,'Helvetica Neue',sans-serif",color:TEXT}}>
      <header style={{position:"sticky",top:0,zIndex:50,background:"rgba(247,246,242,0.96)",backdropFilter:"blur(14px)",borderBottom:`1px solid ${BORDER}`,height:54,display:"flex",alignItems:"center",padding:"0 16px",gap:12}}>
        ...
        <div style={{fontSize:11,color:syncing?"#F59E0B":"#10B981",fontWeight:700,width:20,textAlign:"center"}}>
          {syncing?"↑":"✓"}
        </div>
      </header>
      ...
      {(tab==="jour"||tab==="semaine")&&(
        <button onClick={()=>setModal({type:"add",ds:date,s:"09:00"})} style={{position:"fixed",bottom:28,right:20,width:56,height:56,borderRadius:"50%",background:TEXT,color:BG,border:"none",fontSize:28,cursor:"pointer",zIndex:40,boxShadow:"0 4px 20px rgba(0,0,0,0.18)",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
      )}
...
function DayView({date,setDate,today,routineFor,customFor,onAddAt,onEditCustom,todayTasks}){
  ...
  return (
    <div style={{maxWidth:580,margin:"0 auto",padding:"0 0 120px"}}>
      ...
      {!isToday&&<div style={{textAlign:"center",marginBottom:8}}><button onClick={()=>setDate(today)} style={{background:"none",border:`1px solid ${BORDER}`,color:SUB,borderRadius:20,padding:"4px 14px",fontSize:12,cursor:"pointer",fontWeight:600}}>Aujourd'hui</button></div>}
      ...
      <div style={{margin:"0 16px",background:SURFACE,borderRadius:18,border:`1px solid ${BORDER}`,overflow:"hidden"}}>
        <div style={{position:"relative",height:totalH}}>
            ...
            return (
              <div key={slotMin} style={{position:"absolute",left:0,right:0,top:i*SLOT_H,height:SLOT_H,borderTop:`1px solid ${isHour?BORDER:"#F3F1ED"}`,pointerEvents:"none",zIndex:1}}>
                <div style={{paddingTop:5,paddingLeft:12,fontSize:isHour?11:10,fontWeight:isHour?600:400,color:isHour?SUB:MUTED,userSelect:"none",lineHeight:1,width:58}}>
                  {slotMin%60===0?`${p2(slotMin/60)}h`:`${p2(Math.floor(slotMin/60))}h${p2(slotMin%60)}`}
                </div>
              </div>
            );
          })}
          {routine.map(r=>{
            ...
            const top=minToPx(rStart), height=durToPx(rEnd-rStart);
            const st=ROUTINE_STYLE[r.cat]||ROUTINE_STYLE.autre;
            const inside=custom.filter(c=>{ const cs=tm(c.s); return cs>=rStart&&cs<rEnd; });
            return (
              <div key={r.id} style={{position:"absolute",left:62,right:10,top:top+1,height:height-2,background:st.bg,border:`1.5px solid ${st.border}`,borderRadius:12,zIndex:5,overflow:"hidden",boxSizing:"border-box"}}>
                <div onClick={()=>onAddAt(date,r.s)} style={{display:"flex",alignItems:"center",gap:6,padding:"7px 10px 5px",cursor:"pointer",flexShrink:0}}>
                ...
                {inside.map(c=>{
                  ...
                  const cDur=Math.min(ce,rEnd)-cs;
                  const cTop=minToPx(cs)-top, cHeight=Math.max(durToPx(cDur)-2,SLOT_H*0.8);
                  const cat=CAT[c.cat]||CAT.autre, short=cHeight<SLOT_H*1.2;
                  return (
                    <div key={c.id} onClick={ev=>{ev.stopPropagation();onEditCustom(c);}} style={{position:"absolute",left:4,right:4,top:cTop+2,height:cHeight,background:cat.color,borderRadius:9,padding:short?"5px 10px":"8px 12px",cursor:"pointer",zIndex:10,display:"flex",flexDirection:"column",justifyContent:"center",boxShadow:"0 2px 8px rgba(0,0,0,0.15)",overflow:"hidden",boxSizing:"border-box"}}>
                      <div style={{fontWeight:700,fontSize:short?12:14,color:"#fff",lineHeight:1.2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.title}</div>
                      {!short&&<div style={{fontSize:11,color:"rgba(255,255,255,0.75)",marginTop:2}}>{c.s}{c.s!==c.e?` → ${c.e}`:""}</div>}
                    </div>
                  );
                })}
                <div onClick={()=>onAddAt(date,r.s)} style={{position:"absolute",bottom:6,left:10,fontSize:10,color:st.text,opacity:0.3,cursor:"pointer"}}>+ ajouter</div>
              </div>
            );
          })}
          {custom.filter(c=>{ const cs=tm(c.s); return !routine.some(r=>cs>=tm(r.s)&&cs<tm(r.e)); }).map(c=>{
            const sMin=tm(c.s), eMin=c.s===c.e?sMin+30:tm(c.e);
            if(sMin<SLOT_MIN||sMin>=SLOT_MAX) return null;
            const top=minToPx(sMin), height=Math.max(durToPx(eMin-sMin)-2,SLOT_H*0.9);
            const cat=CAT[c.cat]||CAT.autre, short=height<SLOT_H*1.2;
            return (
              <div key={c.id} onClick={()=>onEditCustom(c)} style={{position:"absolute",left:62,right:10,top:top+1,height:height,background:cat.color,color:"#fff",borderRadius:12,padding:short?"5px 10px":"8px 12px",cursor:"pointer",zIndex:6,display:"flex",flexDirection:"column",justifyContent:"center",boxShadow:"0 2px 8px rgba(0,0,0,0.12)",overflow:"hidden",boxSizing:"border-box"}}>
...
function EventModal({modal,onClose,onSave,onDelete}){
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:100,backdropFilter:"blur(4px)"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:SURFACE,borderRadius:"20px 20px 0 0",width:"100%",maxWidth:580,padding:"20px 20px 44px",maxHeight:"90vh",overflowY:"auto"}}>
        <div style={{width:36,height:4,background:BORDER,borderRadius:2,margin:"0 auto 18px"}}/>
        <div style={{fontWeight:800,fontSize:18,marginBottom:18}}>{isEdit?"Modifier":"Ajouter un événement"}</div>
...
function Fld({label,children}){ return <div style={{marginBottom:14}}><div style={{fontSize:11,fontWeight:700,color:MUTED,letterSpacing:"0.07em",marginBottom:5,textTransform:"uppercase"}}>{label}</div>{children}</div>; }
function inp(){ return {width:"100%",boxSizing:"border-box",background:BG,border:`1px solid ${BORDER}`,borderRadius:10,padding:"11px 13px",fontSize:15,color:TEXT,outline:"none",fontFamily:"inherit"}; }
function arrow(){ return {background:"none",border:`1px solid ${BORDER}`,color:SUB,borderRadius:10,width:38,height:38,fontSize:22,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}; }
...
function TasksView({tasks,setTasks}){
  ...
  return (
    <div style={{maxWidth:560,margin:"0 auto",padding:"16px 16px 100px"}}>
      ...
      {open&&<div style={{background:SURFACE,border:`1px solid ${BORDER}`,borderRadius:14,padding:14,marginBottom:14}}>
      ...
      {done.length>0&&<><div style={{fontSize:11,fontWeight:700,color:MUTED,letterSpacing:"0.08em",margin:"18px 0 8px"}}>TERMINÉES</div>{done.map(t=><TRow key={t.id} task={t} done onToggle={id=>setTasks(p=>p.map(x=>x.id===id?{...x,done:false}:x))} onDelete={id=>setTasks(p=>p.filter(x=>x.id!==id))}/>)}</>}
    </div>
  );
}
